...
if (isset($_GET['addToCart'])) {
    ...
    $check_bill_sql = "SELECT * FROM bills WHERE bill_id = '$bill_id'";
    $bill_result = mysqli_query($link, $check_bill_sql);

    if (mysqli_num_rows($bill_result) > 0) {
        // Bill exists, proceed with adding/updating items
        ...
        // Bill does not exist, display error message
        echo '<script>alert("Error: bill_id does not exist.")</script>';
    }
}
?>
